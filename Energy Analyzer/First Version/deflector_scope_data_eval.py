...
deffolderinp = "/home/timo/Documents/LBL/DataEval/2020-03-11/"
...
def rowfileplot(
    ...
    for n, rfv in enumerate(rfvlots):
        ...
        # add error
        for i in defvolts:
            energylist.append(interpolation.ienergy(i * 2, slitpos))
        ...
        nI = []
        for ch in chs:
            for j, gn in enumerate(range(0, defvolts.__len__())):
                t = []
                ms = []
                results = []
                with open(f"{deffolderinp}{rfv}V/{rfv}V_RF_{gn}.csv") as csvfile:
                ...
                fig, ax = plt.subplots()
                ax.plot(np.array(t) * 1e6, np.array(ms) * 1e3, linewidth=0.1)
                plt.ylim(-5, 220)
        ...
        print(f"plotting length {defvolts.__len__()} :: {nI.__len__()}")
        ...
        ax.set(xlabel="Particle Energy (eV)", ylabel="~ nIons")
        # plt.xlim(1400, 2600)
        plt.ylim(-10, 300)
        plt.xlim(5500, 9900)
        plt.grid(b=True, which="major")
        plt.savefig(f"{deffolderout}{rfv}V.png", dpi=500)
        plt.close()
    # plottin comboplot
    fig, ax = plt.subplots()
    for k in range(collectedx.__len__()):
        ax.plot(collectedx[k], collectedy[k], label=f"RF {rfvlots[k]}V", linewidth=0.5)
    plt.grid(b=True, which="major")
    plt.legend()
    plt.ylim(-10, 300)
    plt.xlim(5500, 9900)
    ...
    plt.savefig(f"{deffolderout}combined.png", dpi=500)
    plt.close()
    ...
    plt.savefig(f"{deffolderout}combined_detail.png", dpi=500)
    plt.close()
...
def singlefileplot(name, ch, note):
    ...
    # plt.yticks(np.arange(-65, 65, 5))
    plt.grid(b=True)
    plt.savefig(f"{deffolderout}{name}.png", dpi=300)
    plt.close()
