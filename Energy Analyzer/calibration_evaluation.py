...
pathtooutput = (
    "/home/timo/Documents/LBL/Warp/atap-meqalac"
...
def byEnergy(nrg):
    ...
    voltages = list()
    xScreen = list()
    files = glob.glob(os.path.join(pathtosims, f"{nrg:.1f}" f"eV_*V_calibration.json"))
    for f in files:
        with open(f, "r") as fp:
            arr = json.load(fp)
            if arr["x"] != []:
...
def byVoltage(volt):
    ...
    xScreen = list()
    energies = list()
    files = glob.glob(os.path.join(pathtosims, f"*eV_{volt:.1f}V_calibration.json"))
    print(files)
    for f in files:
        with open(f, "r") as fp:
            arr = json.load(fp)
...
fig, ax = plt.subplots()
# Adding lines
ax.plot(linerange, [35.865, 35.865], color="black", alpha=0.3)
ax.plot(linerange, [35.865 - 0.5, 35.865 - 0.5], color="black", alpha=0.3)
ax.plot(linerange, [35.865 + 0.5, 35.865 + 0.5], color="black", alpha=0.3)
#
...
ax.set(xlabel="Cap V", ylabel="Screen x (mm)", title="Calibration of Spectrometer")
plt.grid(b=True, which="major")
# plt.yticks(range(25,55,1))
# plt.xticks(range(2900,4600,50), rotation="vertical")
plt.tight_layout()
plt.savefig(f"{pathtooutput}calibration_plot_e", dpi=1000)
# plt.show()
plt.close()


def getAngle(nrg, volt):
    f = f"{pathtosims}{nrg:.1f}eV_" f"{volt:.1f}V_calibration.json"
    with open(f, "r") as fp:
        dic = json.load(fp)
...
def simulationshittingtheslit():
    ...
    min = 35.86 - 10
    acceptedPairs = []
    files = glob.glob(os.path.join(pathtosims, f"*_calibration.json"))
    for f in files:
        with open(f, "r") as fp:
            dic = json.load(fp)
        print(dic["x"][0])
        if min < dic["x"][0] * 1000 < max:
            acceptedPairs.append([dic["energy"], dic["voltage"]])
    print(acceptedPairs)
