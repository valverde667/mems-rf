...
rejectedIons = Species(type=Argon, charge_state=1, name='Ca', color=red)
...
top.rinject = 9999.
top.npinject = 10#300  # needed!! macro particles per time step or cell
...
toffsets = [-1*ns, -3.5*ns, -4.5*ns, 3.5*ns, 3*ns, 1.5*ns, 0, 0, -1*ns, 0]
...
def rfDistances(numRF, Vmax, freq, mass, eInject):
    rfDistances = []
    energy = eInject*selectedIons.charge
    for i in range(numRF):
        energy += .8*Vmax*selectedIons.charge
        velocity = np.sqrt(2*energy/mass)
        distance = velocity/(freq*2)
        rfDistances.append(distance)
    return rfDistances


#ekininitSI=ekininit*ions.charge
distances = rfDistances(numRFesq, Vmax, freq, selectedIons.mass, ekininit)
rfGaps = distances[0::2]
esqGaps = distances[1::2]

j = 0
while j < len(esqGaps):
    gapOrigional = esqGaps[j]
    gapNew = gapOrigional
    while gapNew < 1.54*mm:
        gapNew += 2*gapOrigional
    esqGaps[j] = gapNew
    j += 1
...
print("***************************")
print("***************************")
print('rfGaps ' + str(rfGaps))
print('esqGaps ' + str(esqGaps))
print("***************************")
print("***************************")
Vpos = []
thickness=2*um

for i, toffset, rfgap, gaps in zip(range(numRFesq), toffsets, rfGaps, esqGaps):
    Ekin += 2 * 0.8 * Vmax
...
while (top.time < tmax and zmax < zrunmax):
    ...
    # tmp = "V: {:.0f} {:.0f} {:.0f} {:.0f} {:.0f} {:.0f} {:.0f}".format(Volts[0],
    #                                                              Volts[1],
    #                                                              Volts[2],
    #                                                              Volts[3],
    #                                                              Volts[4],
...
##    tmp = " Voltage: {}V gap: {}um".format(int(Vesq), int(1e6*gap))
    ...
    '''removed plotting routines to make it run faster'''
    '''*********************'''
    Z = rejectedIons.getz()
    if Z.mean() > zmid:
    ...
    #
    # # create some plots
    KE = rejectedIons.getke()
    #
    if len(KE) > 0:
        rejectedIons.ppzke(color=red)
    ...
    rejectedIons.ppzx(color=green, titles=0)
