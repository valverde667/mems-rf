...
#   emitting radius
warpoptions.parser.add_argument(
    "--emit", dest="emittingRadius", type=float, default="1e-3"
)  # .37e-3 #.25

#   divergence angle
warpoptions.parser.add_argument(
    "--diva", dest="divergenceAngle", type=float, default="30e-3"
)
...
now = datetime.now()
# dd/mm/YY H:M:S
dt_string = now.strftime("Run18.3.1_%Y_%m_%d__%H_%M_%S")
...
V_esq = [1.120392e2, -2.939771e2, 3.601633e2, -3.477206e2, 3.254353e2, -2.933683e2, 2.94e2, -3.03e2, 3e2, -3e2]
d_drift = 4e-3
d_esq =1.59e-3 
d_beginning=0
...
length = (esq_positions[-1] + 5 * wp.mm) / 10
tmax = (length / velo ) / 10 # this is used for the maximum time for timesteps
