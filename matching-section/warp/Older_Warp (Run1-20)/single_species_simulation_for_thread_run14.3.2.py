...
#   emitting radius
warpoptions.parser.add_argument(
    "--emit", dest="emittingRadius", type=float, default=".3e-3"
)  # .37e-3 #.25
...
now = datetime.now()
# dd/mm/YY H:M:S
dt_string = now.strftime("Run14.3.1_%Y_%m_%d__%H_%M_%S")
...
from geometry import RF_stack, ESQ_doublet, ESQ
import geometry
...
if loadbeam == "":
    ...
    wp.top.npmax = 1000  # maximal number of particles (for injection per timestep???)
    wp.top.inject = 1  # 2 means space-charge limited injection
    wp.top.npinject = 250  # approximate number of particles injected per step
...
esq0 = ESQ(esq_positions[0], invertPolarity[0], voltages[0])
esq1 = ESQ(esq_positions[1], invertPolarity[1], voltages[1])
esq2 = ESQ(esq_positions[2], invertPolarity[2], voltages[2])
esq3 = ESQ(esq_positions[3], invertPolarity[3], voltages[3])
#esq4 = ESQ(esq_positions[4], invertPolarity[4], voltages[4])
#esq5 = ESQ(esq_positions[5], invertPolarity[5], voltages[5])
#esq6 = ESQ(esq_positions[6], invertPolarity[6], voltages[6])
#esq7 = ESQ(esq_positions[7], invertPolarity[7], voltages[7])
#esq8 = ESQ(esq_positions[8], invertPolarity[8], voltages[8])
#esq9 = ESQ(esq_positions[9], invertPolarity[9], voltages[9])


conductors=esq0+esq1+esq2+esq3#+esq4#+esq5+esq6+esq7+esq8+esq9
...
for esq_pos in esq_positions:
    ...
    child_1 = solver.addchild(
        mins=[wp.w3d.xmmin, wp.w3d.ymmin, esq_pos ,],
        maxs=[wp.w3d.xmmax, wp.w3d.ymmax, esq_pos ,],
        refinement=[1, 1, 5],
    )
...
wp.pfxy(fill=1, filled=1, plotphi=0)  # plots contours of potential
wp.fma()  # second frame in cgm file

wp.pfxy(fill=1, filled=1, plotphi=1)  # plots contours of potential
wp.fma()  # second frame in cgm file

wp.pfxy(fill=1, plotphi=1)  # plots contours of potential
wp.fma()  # second frame in cgm file

wp.pfxyg(fill=1, filled=1, plotphi=1)  # plots contours of potential
wp.fma()  # second frame in cgm file

wp.pfxyg(fill=1, comp='E', plotphi=1)  # plots contours of potential
wp.fma() 
wp.pfxyg(fill=1, comp='x', plotphi=1)  # plots contours of potential
wp.fma() 
wp.pfxyg(fill=1, comp='y', plotphi=1)  # plots contours of potential
wp.fma() 
wp.pfxyg(fill=1, comp='z', plotphi=1)  # plots contours of potential
wp.fma() 

wp.pfxy(fill=1, filled=1, plotphi=1, plotselfe=True)  # plots contours of potential
wp.fma()  # second frame in cgm file
