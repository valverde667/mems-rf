...
from geometry import RF_stack, ESQ
import geometry
...
#conductors = ESQ(esq_positions, invertPolarity, voltages)
#wp.installconductors(conductors)
#wp.fieldsol(-1)
esqs=[]
conductors=[]
for i in range(len(esq_positions)):
	esqs += ESQ(position=esq_positions[i], invertPolarity=invertPolarity[i], voltage=voltages[i])

wp.installconductors(conductors)
wp.fieldsol(-1)
# creat submesh for ESQ: maybe want to add back later
meshes = []
...
length = esq_positions[-1] + 25 * wp.mm
