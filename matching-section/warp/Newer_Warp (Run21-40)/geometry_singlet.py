...
def ESQ_singlet(position, invertPolarity, voltage):
    """
        One ESQ wafer
        :param position: of the center of a single ESQ wafer
        :param invertPolarity: inverts polarity,
    ...
    # Dimensions:
    d_beamhole = 1.1 * wp.mm
    r_beamhole = d_beamhole / 2
    wafer_thickness = 1.59 * wp.mm
    metal_width = 0.1 * wp.mm
    r_out_beamhole = r_beamhole + metal_width
    d_out_beamhole = 2 * r_out_beamhole
    d_electrode = 1.3 * wp.mm
    r_electrode = d_electrode / 2
    r_out_electrode = r_electrode + metal_width
    d_out_electrode = 2 * r_out_electrode
    xcent_electrode = r_beamhole + metal_width + r_electrode
    ycent_electrode = r_beamhole + metal_width + r_electrode
    ...
    electrode_annulus0 = wp.ZAnnulus(
            rmin=r_electrode, 
            rmax=r_out_electrode,
            length=wafer_thickness,
            voltage=voltage * vsigns[0]*invertPolarity,
            xcent=-xcent_electrode,
            ycent=0,
            zcent=position,
            )
    electrode_annulus1 = wp.ZAnnulus(
            rmin=r_electrode,
            rmax=r_out_electrode,
            length=wafer_thickness,
            voltage=voltage * vsigns[1]*invertPolarity,
            xcent=0,
            ycent=-ycent_electrode,
            zcent=position,
            )
    electrode_annulus2 = wp.ZAnnulus(
            rmin=r_electrode,
            rmax=r_out_electrode,
            length=wafer_thickness,
            voltage=voltage * vsigns[2]*invertPolarity,
            xcent=xcent_electrode,
            ycent=0,
            zcent=position,
            )
    electrode_annulus3 = wp.ZAnnulus(
            rmin=r_electrode,
            rmax=r_out_electrode,
            length=wafer_thickness,
            voltage=voltage * vsigns[3]*invertPolarity,
            xcent=0,
            ycent=ycent_electrode,
            zcent=position,
            )       
  
    return (electrode_annulus0+electrode_annulus1+electrode_annulus2+electrode_annulus3)
