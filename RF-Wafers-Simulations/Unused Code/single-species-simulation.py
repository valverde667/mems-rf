...
a = (
    ...
    e
) = f = g = 0  # placeholder values to keep track of how many parameters were changed
...
wp.w3d.ymmin = -0.02 / 8.0 * 1.2  # toDo why is this not the
# same as x???
wp.w3d.ymmax = +0.02 / 8.0 * 1.2
wp.w3d.zmmin = 0.0
wp.w3d.zmmax = (
    23 * wp.mm
)  # 53*wp.mm #changes the length of the gist output window. Maybe this should be the spread of the last particles in the simulation to the end of the last acceleration gap
...
energies = [
    ekininit + V_arrival * Vmax * (i + 1) for i in range(numRF)
]  # (i+1) to start at proper 15,000 energy gain, Vmax needs to be adjusted depending on the actual strength of the electric field in the acceleration gap, the middle of the gaps are lower than the vmax for grants design
distances = [
    wp.sqrt(energy * selectedIons.charge / (2 * speciesMass)) * 1 / freq
...
geometry.pos = (
    -0.5 * distances[0]
    - 0.5 * geometry.RF_gap
    - geometry.RF_thickness
    - 2 * geometry.copper_thickness
    + drifti
...
t_offset = (
    ...
    - 0.5 * geometry.RF_gap
    - 2 * geometry.copper_thickness
) / veloinit  # phase offset to account for moving the injection beam farther to the left
print("TIME OFFSET = {}".format(t_offset))  # not the same as 8e-9
RF_toffset = (
    np.arcsin(V_arrival) / (2 * np.pi * freq) - (d_mid + drifti) / veloinit + 8e-9
)  # .5*L_bunch - 1e-9# L_bunch - 8e-9
...
for i, bl2s in enumerate(pairwise(zip(distances, energies))):
    (
        (rf_bl2, E1),
        (esq_bl2, E2),
    ) = bl2s  # calling distances rf_bl2s # #calling distances rf_bl2s
    # use first betalamba_half for the RF unit
    RF = RF_stack3(
        condid=[ID_RF, ID_RF + 1, ID_RF + 2, ID_RF + 3],
        betalambda_half=rf_bl2,
        voltage=gen_volt(RF_toffset),
    )
    ...
    new_start_RF_stack = geometry.pos + (
        rf_bl2
    )  # - 4*geometry.copper_thickness - 2*geometry.RF_thickness)
    print(f"I am actually starting the next wafer stack here at {new_start_RF_stack}")
    Gap(
        rf_bl2
        - geometry.RF_gap
        - 4 * geometry.copper_thickness
        - 2 * geometry.RF_thickness
    )  # put this into geometry??????? with different position for the ESQ
...
conductors = wp.sum(
    RFs
)  # + spectrometer #+ wp.sum(ESQs)# + names all ESQs and RFs conductors in order to feed into warp
...
length = (
    geometry.pos + 2.5 * wp.cm
)  # 2.5mm added to allow particles to completely pass through last RF gap, cant call targetz before the acceleration gaps are made
...
while wp.top.time < tmax and zmax < zrunmax:
    ...
    wp.top.pline1 = "V_RF: {:.0f}".format(gen_volt(RF_toffset)(wp.top.time))
    ...
    if (
        Z.mean() > zmid
    ):  # if the mean distance the particles have travelled is greater than the middle of the frame do this:
        wp.top.vbeamfrm = (
    ...
    wp.pfzx(
        ...
        comp="z",
        titles=0,
        cmin=-1.2 * Vmax / geometry.RF_gap,
        cmax=1.2 * Vmax / geometry.RF_gap,
    )  # Vmax/geometry.RF_gap #1.2*Vmax/geometry.RF_gap (if want to see 20% increase in electric field) #comp='z': the component of the electric field to plot, 'x', 'y', 'z' or 'E',use 'E' for the magnitude.
    ...
    if gap_num_select < len(distances) and len(selectedIons.getz() > 0):
        if (
            np.max(selectedIons.getz())
            > np.cumsum(distances)[gap_num_select] - 0.5 * distances[0]
        ):
...
with open(
    f"{parameter_name}__{change}__{datetimestamp}__surviving_particles.json", "w"
) as write_file:
    json.dump(json_data, write_file, indent=2)

# open the file that was just created
with open(
    f"{parameter_name}__{change}__{datetimestamp}__surviving_particles.json"
) as f:
    data = json.load(f)
...
with open(
    f"{parameter_name}__{change}__{datetimestamp}__ZCrossing.json", "w"
) as write_file:
...
os.system(
    f"mv {parameter_name}__{change}__{datetimestamp}.000.cgm {atap_path}/{parameter_name}/"
)
os.system(
    f"mv {parameter_name}__{change}__{datetimestamp}__surviving_particles.json {atap_path}/{parameter_name}/"
)
os.system(
    f"mv {parameter_name}_esqhist_{datetimestamp2}.npy {atap_path}/{parameter_name}/"
)
os.system(
    f"mv {parameter_name}__{change}__{datetimestamp}__ZCrossing.json {atap_path}/{parameter_name}/"
)
os.system(
    f"gist -x {atap_path}/{parameter_name}/{parameter_name}__{change}__{datetimestamp}.000.cgm"
)
