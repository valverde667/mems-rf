  ...
  Various methods are accessible if additional fine control is needed.
    ...
    def __init__(
        ...
        self.lsavecondid = (
            ...
            or lsaveintercept
            or lrefineintercept
            or lrefineallintercept
            or lcollectlpdata
        )
        assert not (
            self.lsavecondid and self.lfastscraper
        ), "With the fast scraper, the conductor information where the particles are lost is not calculated and so lsavecondid will not work"
    ...
    def registerconductors(self, newconductors):
        ...
        for c in newconductors:
            assert (
                c.condid > 0
            ), "The conductor id must be greater than zero in order for the particle scraping to work."
    ...
    data needed by the grid is set up. This will normally be called automatically.
        ...
        if lforce or not self.usergrid:
            ...
            if not lforce:
                ...
                tdc = top.ppdecomp
                gridchanged = not (
                    self.grid.nxlocal == w3d.nxp * self.nxscale
                    and self.grid.nylocal == w3d.nyp * self.nyscale
                    and self.grid.nzlocal == w3d.nzp * self.nzscale
                    and self.grid.xmmin == w3d.xmmin
                    and self.grid.xmmax == w3d.xmmax
                    and self.grid.ymmin == w3d.ymmin
                    and self.grid.ymmax == w3d.ymmax
                    and self.grid.zmmin == w3d.zmmin
                    and self.grid.zmmax == w3d.zmmax
                    and gdc.ix[gdc.ixproc] == tdc.ix[tdc.ixproc] * self.nxscale
                    and gdc.iy[gdc.iyproc] == tdc.iy[tdc.iyproc] * self.nyscale
                    and gdc.iz[gdc.izproc] == tdc.iz[tdc.izproc] * self.nzscale
                    and gdc.nx[gdc.ixproc] == tdc.nx[tdc.ixproc] * self.nxscale
                    and gdc.ny[gdc.iyproc] == tdc.ny[tdc.iyproc] * self.nyscale
                    and gdc.nz[gdc.izproc] == tdc.nz[tdc.izproc] * self.nzscale
                    and (self.zbeamprev == top.zbeam or self.gridmode != 0)
                )
            else:
                gridchanged = 1
            ...
            if gridchanged:
                ...
                self.grid = Grid(
                    decomp=copy.deepcopy(top.ppdecomp),
                    nxscale=self.nxscale,
                    nyscale=self.nyscale,
        ...
        if self.newconductors or gridchanged:
            ...
            if top.chdtspid > 0:
                if (
                    ...
                    or w3d.nyc != self.grid.nylocal
                    or w3d.nzc != self.grid.nzlocal
                ):
                    w3d.nxc = self.grid.nxlocal
    ...
    before it was lost is needed."""
        ...
        if not (self.lsaveoldpositions or self.lsaveoldvelocities):
            return
        ...
        for js in self.jslist():
            if top.pgroup.ldts[js] and top.pgroup.nps[js] > 0:
                ...
                if self.lsaveoldpositions:
                    ...
                    top.pgroup.pid[i1:i2, self.oldisOK] = 1.0
        ...
        for js in self.jslist(jslist):
            if top.pgroup.ldts[js]:
                ...
                if self.l_print_timing:
                    tend = wtime()
                if self.l_print_timing:
                    print js, "scrape", tend - tstart
                if self.l_print_timing:
                    tstart = wtime()
                if clear or self.lsavecondid:
                    processlostpart(
                        top.pgroup, js + 1, top.clearlostpart, top.time, top.zbeam
                    )
                if self.l_print_timing:
                    tend = wtime()
                if self.l_print_timing:
                    print js, "processlosspart", tend - tstart
                if self.l_print_timing:
                    tstart = wtime()
                if self.lsavecondid:
                ...
                if self.l_print_timing:
                    tend = wtime()
                if self.l_print_timing:
                    print js, "savecondid", tend - tstart
    ...
    def scrape(self, js):
        ...
        if w3d.solvergeom in [w3d.XYZgeom]:
            ...
            # --- the original particle position is used below.
            rr = sqrt(xx ** 2 + yy ** 2)
            getgrid1d(top.pgroup.nps[js], rr, pp, nx, isinside[:, iya, iza], xmin, xmax)
        elif w3d.solvergeom == w3d.Ygeom:
            xsym, ysym = self.applysymmetry(0, yy)
            getgrid1d(
                top.pgroup.nps[js], ysym, pp, ny, isinside[ixa, :, iza], ymin, ymax
            )
        elif w3d.solvergeom == w3d.Zgeom:
            getgrid1d(top.pgroup.nps[js], zz, pp, nz, isinside[ixa, iya, :], ymin, ymax)
        else:
            raise Exception(
                "The particle scraping only works for XYZ, XY, RZ, R, Y and Z geometry"
            )
        ...
        iclose = compress(pp > 0.0, arange(i1, i2))
        if len(iclose) == 0:
            return
        ...
        if w3d.solvergeom in [w3d.XYZgeom]:
            nd = 3
            gdx = [0.0, dx, 0.0, dx, 0.0, dx, 0.0, dx]
            gdy = [0.0, 0.0, dy, dy, 0.0, 0.0, dy, dy]
            gdz = [0.0, 0.0, 0.0, 0.0, dz, dz, dz, dz]
        ...
        for i in range(2 ** nd):
            ...
            for c in self.conductors:
                ...
                if self.lrefineallintercept:
                    ...
                    closeenough = logical_or(
                        (maxchange > distance.distance), currentisinside
                    )
                    ...
                    if len(iclose2) > 0:
                        put(
                            top.pgroup.pid[:, self.xoldpid],
                            iclose2,
                            compress(refinedisinside, xo),
                        )
                        put(
                            top.pgroup.pid[:, self.yoldpid],
                            iclose2,
                            compress(refinedisinside, yo),
                        )
                        put(
                            top.pgroup.pid[:, self.zoldpid],
                            iclose2,
                            compress(refinedisinside, zo),
                        )
                ...
                if c.material == "reflector":
                    ...
                    if len(icnew) > 0:
                        put(top.pgroup.gaminv, icnew, 0.0)
                    ...
                    if self.lsaveoldvelocities:
                        ...
                        put(
                            top.pgroup.uxp,
                            ic,
                            -take(top.pgroup.pid[:, self.uxoldpid], ic),
                        )
                        put(
                            top.pgroup.uyp,
                            ic,
                            -take(top.pgroup.pid[:, self.uyoldpid], ic),
                        )
                        put(
                            top.pgroup.uzp,
                            ic,
                            -take(top.pgroup.pid[:, self.uzoldpid], ic),
                        )
                    else:
    ...
    counting the current lost on the conductor.
        ...
        xg = (
            xmin
            + aint(abs(x8 - xmin) / dx) * dx
            + array(int(nn / 8) * [0.0, dx, 0.0, dx, 0.0, dx, 0.0, dx])
        )
        yg = (
            ymin
            + aint(abs(y8 - ymin) / dy) * dy
            + array(int(nn / 8) * [0.0, 0.0, dy, dy, 0.0, 0.0, dy, dy])
        )
        zg = (
            zmin
            + aint(abs(z8 - zmin) / dz) * dz
            + array(int(nn / 8) * [0.0, 0.0, 0.0, 0.0, dz, dz, dz, dz])
        )
        pp = zeros(nn, "d")
        ...
        if w3d.solvergeom in [w3d.XYZgeom]:
            ...
            xgsym, ygsym = self.applysymmetry(xg, 0)
            getgridngp1d(
                nn, xgsym, pp, nx, self.reducedisinside[:, iya, iza], xmin, xmax
            )
        elif w3d.solvergeom == w3d.Ygeom:
            xgsym, ygsym = self.applysymmetry(0, yg)
            getgridngp1d(
                nn, ygsym, pp, ny, self.reducedisinside[ixa, :, iza], ymin, ymax
            )
        elif w3d.solvergeom == w3d.Zgeom:
            getgridngp1d(nn, zg, pp, nz, self.reducedisinside[ixa, iya, :], zmin, zmax)
        else:
            raise Exception(
                "The particle scraping only works for XYZ, XZ, XY and RZ geometry"
            )
        ...
        for c in self.conductors:
            ...
            if self.lsaveintercept:
                ...
                if self.interceptvelocitymethod == "finitedifference":
                    ...
                    gi = 1.0 / sqrt(1.0 + (ux ** 2 + uy ** 2 + uz ** 2) / clight ** 2)
                    vx = ux * gi
                    vy = uy * gi
                    vz = uz * gi
                else:
                    raise ValueError(
                        "interceptvelocitymethod has an incorrect value, %s"
                        % self.interceptvelocitymethod
                    )

                # --- get v in lab frame
                if top.boost_gamma > 1.0:
                    boost_beta = -sqrt(1.0 - 1.0 / top.boost_gamma ** 2)
                    fact = 1.0 / (1.0 - boost_beta * vz / clight)
                ...
                if top.lrelativ:
                    ...
                    # --- when beta == 0.
                    betacorrection = 1.0 / where(beta >= 0.99999, beta / 0.99999, 1.0)
                ...
                put(top.gaminvlost, ic, 1.0 / gamma)
    ...
    particle).
        ...
        # --- per cyclotron orbit.
        isteps = nint(dt * omegac / (2.0 * pi) * self.nstepsperorbit)
    ...
    luserefinedifnotlost: when true, if the refined particle orbit is not lost,
        ...
        for it in range(nsteps):
            ...
            # --- input arrays.
            bpusht3d(
                nn, uxo, uyo, uzo, gaminv, bx, by, bz, q, m, dtover, 0.5, top.ibpush
            )
            ...
            if top.pboundxy == periodic:
                ...
                xc[:] = where(xc > w3d.xmmax, 2.0 * w3d.xmmax - xc, xc)
                xc[:] = where(xc < w3d.xmmin, 2.0 * w3d.xmmin - xc, xc)
                yc[:] = where(yc > w3d.ymmax, 2.0 * w3d.ymmax - yc, yc)
                yc[:] = where(yc < w3d.ymmin, 2.0 * w3d.ymmin - yc, yc)
            ...
            if top.pboundnz == reflect:
                ...
                zc[:] = where(zc > zmmax, 2.0 * zmmax - zc, zc)
            if top.pbound0 == reflect:
                uzo[:] = where(zc < zmmin, -uzo, uzo)
                zc[:] = where(zc < zmmin, 2.0 * zmmin - zc, zc)
            ...
            dtover = where(isinside, 0.0, dtover)
        ...
        # --- in the loop above.
        if sometrue(dtover > 0.0):
            userefined = (dtover == 0.0) | luserefinedifnotlost
            ...
            if itime is not None:
                itime[:] = where(userefined, itime, 0.0)
    ...
    interpolating errors from the grid. This will normally be called automatically.
        ...
        if w3d.solvergeom in [w3d.XYZgeom]:
            ...
            # --- the original particle position is used below.
            rr = sqrt(xx ** 2 + yy ** 2)
            getgrid1d(top.pgroup.nps[js], rr, pp, nx, distances[:, 0, 0], xmin, xmax)
        elif w3d.solvergeom == w3d.Ygeom:
            xsym, ysym = self.applysymmetry(0, yy)
            getgrid2d(top.pgroup.nps[js], ysym, pp, ny, distances[0, :, 0], ymin, ymax)
        elif w3d.solvergeom == w3d.Zgeom:
            getgrid2d(top.pgroup.nps[js], zsym, pp, nz, distances[0, 0, :], zmin, zmax)
        else:
            raise Exception(
                "The particle scraping only works for XYZ, XY and RZ geometry"
            )
        ...
        ilost = compress(pp < 0.0, arange(i1, i2))
        if len(ilost) == 0:
            return
        ...
        if self.reflectiveconductors:
            ...
            if self.lsaveoldvelocities:
                ...
                put(
                    top.pgroup.uxp,
                    ilost,
                    -take(top.pgroup.pid[:, self.uxoldpid], ilost),
                )
                put(
                    top.pgroup.uyp,
                    ilost,
                    -take(top.pgroup.pid[:, self.uyoldpid], ilost),
                )
                put(
                    top.pgroup.uzp,
                    ilost,
                    -take(top.pgroup.pid[:, self.uzoldpid], ilost),
                )
            else:
                # --- Otherwise use the new velocity. Can this lead to errors?
            ...
            # --- For particles which are inside, set gaminv to 0, the lost
            # --- particle flag
            put(top.pgroup.gaminv, ilost, 0.0)
    ...
    def pdxy(self, iz=0, fullplane=0, xyantisymmetric=0, **kw):
        ...
        ppgeneric(
            grid=data,
            xmin=self.grid.xmmin,
            xmax=self.grid.xmmax,
            ymin=self.grid.ymmin,
            ymax=self.grid.ymmax,
            **kw
        )
    ...
    def pdzx(self, iy=0, fullplane=0, **kw):
        ...
        self.grid.solvergeom = w3d.solvergeom
        data = getdecomposedarray(
            data, iy=iy, bcast=0, local=0, fullplane=fullplane, solver=self.grid
        )
        del self.grid.fsdecomp
        del self.grid.solvergeom
        ppgeneric(
            gridt=data,
            xmin=self.grid.zmminlocal,
            xmax=self.grid.zmmaxlocal,
            ymin=self.grid.xmminlocal,
            ymax=self.grid.xmmaxlocal,
            **kw
        )
    ...
    def pdzy(self, ix=0, fullplane=0, **kw):
        ...
        self.grid.solvergeom = w3d.solvergeom
        data = getdecomposedarray(
            data, ix=ix, bcast=0, local=0, fullplane=fullplane, solver=self.grid
        )
        del self.grid.fsdecomp
        del self.grid.solvergeom
        ppgeneric(
            ...
            xmin=self.grid.zmminlocal,
            xmax=self.grid.zmmaxlocal,
            ymin=self.grid.ymminlocal,
            ymax=self.grid.ymmaxlocal,
            **kw
        )
