...
with PdfPages(f"continuous_phase-space-plots_{date_string}.pdf") as pdf:
    ...
    plt.text(0.5, 0.8, rf"Synchronous Phase: {design_phase/np.pi:.3f} $\pi$")
    plt.text(0.5, 0.7, f"Gap Voltage: {design_gap_volt/kV:.2f} [kV]")
    plt.text(0.5, 0.6, f"RF Frequency: {design_freq/MHz:.2f} [MHz]")
    plt.text(
        0.5, 0.5, rf"Beam Length: {(particle_dist[-1] - particle_dist[0])/mm:.2f} [mm]"
    )
    plt.tight_layout()
    pdf.savefig()
    plt.close()
    ...
    for i in range(Np):
        fig, ax = plt.subplots()
        ax.set_title(
            rf"Phase Space for Gap {i}, ${{\cal E}}_s$ = {dsgn_E[i]/kV:.2f} [keV]"
        )
