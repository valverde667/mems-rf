# Decomissioned script. This script used the old classes and functions to find
# matchign conditions in the other script. I thought it useful to save these and
# revert back to if needed. Also, I put alot of work in and didn't just want to
# remove it ha =).
...
if do_accel_section:
    ...
    if do_accel_section_optimization:
        ...
        accel_rp_norm = 1 * mrad
        accel_norms = np.array(
            [3.0 / rp, 3.0 / rp, 1 / accel_rp_norm, 1 / accel_rp_norm]
        )
        accel_parameters = {
            "emit": accel_emit[0],
            "Q": accel_Q[0],
        ...
        ax.plot(
            accel_z / mm, accel_opt_soln_matrix[:, 1] / mm, label=r"$r_y(s)$", c="b"
        )
        ax.scatter(accel_z[-1] / mm, accel_opt_x0[0] / mm, marker="*", c="k", s=90)
        ...
        print(f"Mean rx: {np.mean(accel_opt_soln_matrix[:,0]/mm):2f}")
        print(f"Mean rx: {np.mean(accel_opt_soln_matrix[:,1]/mm):2f}")
