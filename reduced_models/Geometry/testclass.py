...
class Ring(object):
    def __init__(self, rmin=1 * mm, rmax=1.5 * mm, length=2 * mm, zcent=0):
        self.rmin = rmin
        self.rmax = rmax
        self.length = length
        self.zcent = zcent
    ...
    def createconductor(self, voltage):
        conductor = wp.ZAnnulus(
            rmin=rinner, rmax=router, length=length, zcent=zc1, voltage=voltage
        )
...
plate1 = wp.ZAnnulus(rmin=rinner, rmax=router, length=length, zcent=zc1, voltage=V)
plate2 = wp.ZAnnulus(rmin=rinner, rmax=router, length=length, zcent=zc2, voltage=-V)
